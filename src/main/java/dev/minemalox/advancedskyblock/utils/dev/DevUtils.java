package dev.minemalox.advancedskyblock.utils.dev;
...
import dev.minemalox.advancedskyblock.utils.nifty.RegexUtil;
...
public class DevUtils {
    ...
    public static void copyScoreboardSidebar(Scoreboard scoreboard, boolean stripControlCodes) {
        Utils utils = AdvancedSkyblock.getInstance().getUtils();
        ...
        if (scores == null || scores.isEmpty()) {
            AdvancedSkyblock.getInstance().getUtils().sendErrorMessage("No scores were found!");
        } else {
    ...
    public static void copyMobData(EntityPlayerSP player, List<Entity> loadedEntities) {
        ...
        loadedEntitiesCopy.removeIf(entity -> entity.getDistance(player) > ENTITY_COPY_RADIUS ||
    ...
    public static void copyNBTTagToClipboard(NBTBase nbtTag, String message) {
        if (nbtTag == null) {
            AdvancedSkyblock.getInstance().getUtils().sendMessage("This item has no NBT data.");
            return;
        }
    ...
    public static String prettyPrintNBT(NBTBase nbt) {
        ...
        if (tagID == Constants.NBT.TAG_END) {
            ...
            stringBuilder.append('{');
            if (!nbtTagCompound.isEmpty() /* TODO: right? hasNoTags()*/) {
    ...
    private static void writeToClipboard(String text, String successMessage) {
        ...
        try {
            ...
            AdvancedSkyblock.getInstance().getUtils().sendMessage(successMessage);
        } catch (IllegalStateException exception) {
            AdvancedSkyblock.getInstance().getUtils().sendErrorMessage("Clipboard not available.");
        }
    }
}
