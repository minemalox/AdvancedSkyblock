...
public enum EnchantedItemBlacklist {
     ...
     */
    public static boolean shouldBlockUsage(ItemStack itemStack, PlayerInteractEvent action) {
        ...
        for (EnchantedItemBlacklist blacklistItem : EnchantedItemBlacklist.values()) {
            if (main.getConfigValues().isEnabled(blacklistItem.feature) && !blacklistItem.isBucket() &&
                ...
                if (action instanceof PlayerInteractEvent.RightClickEmpty && blacklistItem.onlyBlockPlacement) continue;
     ...
     * @see PlayerListener#onBucketEvent(FillBucketEvent)
     */
