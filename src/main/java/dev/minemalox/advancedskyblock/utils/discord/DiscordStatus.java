package dev.minemalox.advancedskyblock.utils.discord;

import dev.minemalox.advancedskyblock.AdvancedSkyblock;
import dev.minemalox.advancedskyblock.gui.buttons.ButtonSelect;
import dev.minemalox.advancedskyblock.utils.Attribute;
import dev.minemalox.advancedskyblock.utils.Message;
import dev.minemalox.advancedskyblock.utils.SkyblockDate;
import dev.minemalox.advancedskyblock.utils.TextUtils;
...
public enum DiscordStatus implements ButtonSelect.SelectItem {
    ...
    LOCATION(Message.DISCORD_STATUS_LOCATION_TITLE, Message.DISCORD_STATUS_LOCATION_DESCRIPTION,
            () -> AdvancedSkyblock.getInstance().getUtils().getLocation().getScoreboardName()),

    PURSE(Message.DISCORD_STATUS_PURSE_TITLE, Message.DISCORD_STATUS_PURSE_DESCRIPTION,
            () -> String.format("%s Coins", TextUtils.formatDouble(AdvancedSkyblock.getInstance().getUtils().getPurse()))),

    STATS(Message.DISCORD_STATUS_STATS_TITLE, Message.DISCORD_STATUS_STATS_DESCRIPTION,
            () -> {
                int health = AdvancedSkyblock.getInstance().getUtils().getAttributes().get(Attribute.HEALTH).getValue();
                int defense = AdvancedSkyblock.getInstance().getUtils().getAttributes().get(Attribute.DEFENCE).getValue();
                int mana = AdvancedSkyblock.getInstance().getUtils().getAttributes().get(Attribute.MANA).getValue();
//                return String.format("%d\u2764 %d\u2748 %d\u270E", health, defense, mana);
    ...
    ZEALOTS(Message.DISCORD_STATUS_ZEALOTS_TITLE, Message.DISCORD_STATUS_ZEALOTS_DESCRIPTION,
            () -> String.format("%d Zealots killed", AdvancedSkyblock.getInstance().getPersistentValues().getKills())),

    ITEM(Message.DISCORD_STATUS_ITEM_TITLE, Message.DISCORD_STATUS_ITEM_DESCRIPTION,
            () -> {
                final EntityPlayerSP player = Minecraft.getMinecraft().player;
                if (player != null && player.getHeldItem(EnumHand.MAIN_HAND) != null) {
                    return String.format("Holding %s", TextUtils.stripColor(player.getHeldItem(EnumHand.MAIN_HAND).getDisplayName()));
                }
    ...
    TIME(Message.DISCORD_STATUS_TIME_TITLE, Message.DISCORD_STATUS_TIME_DESCRIPTION,
            () -> {
                final SkyblockDate date = AdvancedSkyblock.getInstance().getUtils().getCurrentDate();
                return date != null ? date.toString() : "";
            }),

    PROFILE(Message.DISCORD_STATUS_PROFILE_TITLE, Message.DISCORD_STATUS_PROFILE_DESCRIPTION,
            () -> {
                String profile = AdvancedSkyblock.getInstance().getUtils().getProfileName();
