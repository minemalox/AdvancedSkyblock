...
public class InventoryUtils {
    ...
    public InventoryUtils(AdvancedSkyblock main) {
        this.main = main;
    }
     ...
     */
    public void getInventoryDifference(NonNullList<ItemStack> currentInventory) {
    ...
    public void checkIfInventoryIsFull(Minecraft mc, EntityPlayerSP p) {
        if (main.getUtils().isOnSkyblock() && main.getConfigValues().isEnabled(Feature.FULL_INVENTORY_WARNING)) {
            ...
            for (int i = 0; i < p.inventory.mainInventory.size(); i++) {
                ...
                if (p.inventory.mainInventory.get(i).isEmpty() /*== null*/ && i != 8) {
    ...
    public void checkIfWearingSlayerArmor(EntityPlayerSP p) {
        if (main.getConfigValues().isEnabled(Feature.SLAYER_INDICATOR)) {
            for (int i = 3; i > -1; i--) {
                ItemStack item = p.inventory.armorInventory.get(i);
