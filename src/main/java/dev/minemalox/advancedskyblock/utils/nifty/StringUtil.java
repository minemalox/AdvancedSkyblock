package dev.minemalox.advancedskyblock.utils.nifty;
...
public class StringUtil {
    ...
    public static String implode(String glue, Collection<String> collection, int start, int end) {
        ...
        if (start > collection.size())
            throw new IndexOutOfBoundsException(String.format("Cannot access index %d out of %d total pieces!", start, collection.size()));
        if (end < 0) end = collection.size();
        if (end > collection.size())
            throw new IndexOutOfBoundsException(String.format("Cannot access index %d out of %d total pieces!", end, collection.size()));
