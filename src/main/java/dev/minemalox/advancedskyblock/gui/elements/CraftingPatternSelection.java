...
public class CraftingPatternSelection {
    ...
    public CraftingPatternSelection(Minecraft mc, int x, int y) {
        ...
        PersistentValues persistentValues = AdvancedSkyblock.getInstance().getPersistentValues();
    ...
    public void draw() {
        ...
        for (CraftingPattern craftingPattern : CraftingPattern.values()) {
            ...
            if (craftingPattern != AdvancedSkyblock.getInstance().getPersistentValues().getSelectedCraftingPattern()) {
        ...
        for (CraftingPattern craftingPattern : CraftingPattern.values()) {
            ...
            if (mouseY > this.y + offset && mouseY < this.y + offset + ICON_SIZE) {
                if (persistentValues.getSelectedCraftingPattern() != craftingPattern) {
                    AdvancedSkyblock.getInstance().getUtils().playLoudSound(SoundEvents.UI_BUTTON_CLICK, 1F);
